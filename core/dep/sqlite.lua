...
--[[@return sqlite? database, string? error]]
--[[@param path string]]
...
--[[@return true? success, string? error]]
--[[@param sql string]]
...
sqlite.execute = function(self, sql, ...)
	...
	while true do
		if sqlite_ffi.sqlite3_prepare_v2(self.db[0], c_sql, -1, stmt_ptr, next_sql) ~= 0 then
			return nil,
					"sqlite: sqlite3_prepare_v2: " .. err(self)
		end
...
--[[@return (fun():...:unknown)? iterator, string? error]]
--[[@param sql string]]
--[[@param ... number|string|boolean?]]
	...
	return function()
		...
		if code == 100 then --[[SQLITE_ROW]]
			...
			if #columns > 0 then
				...
				return nil, "sqlite: no columns"
			end
		elseif code == 101 then
			sqlite_ffi.sqlite3_finalize(stmt); return nil, "sqlite: done" --[[SQLITE_DONE]]
		else
			return nil, "sqlite: sqlite3_step: unhandled code " .. code
		end
	end
end
